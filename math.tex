...
\section{math}
    \subsection{general}
        ...
        \begin{tikzpicture}[domain=0:1]
            \draw[very thin,color = gray](-0.1,-1.1) grid(1,1); 
            \draw[->] (-0.2,-1) -- (2.25,-1) node[right] {$x$};
            \draw[->] (0,-1.2) -- (0,1.25) node[above] {$f(x)$};
            \draw plot [color=red,domain=0.1:1,samples=100] (\x,{log2(\x)});
        \end{tikzpicture}
    \subsection{calculus}
