...
\section{math}
    \subsection{general}
        ...
        \subsubsection{Newton's iteration}
            Newton's iteration is a way to approximate the root of a function, given $\dfrac{\Delta y}{\Delta x}$ at $f(x)$.
            \begin{center}
                \begin{tikzpicture}[domain=0:4.71238898038]
                    \draw[very thin, color=gray](-0.1,-1.1)grid (4.8,4.8);
                    \draw[->] (-0.2,0) -- (4.8,0) node[right] {$x$};
                    \draw[->] (0,-1.10) -- (0,4.8) node[right] {$y$};
